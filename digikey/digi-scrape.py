...
type DACType = 
replacements = [
	' (0.154", 3.90mm Width)', ' (0.173", 4.40mm Width)', [' (0.118", 3.00mm Width)', ' (2012 Metric)', 'Concave',
	' (1608 Metric)', ' (3216 Metric)'
]
...
def search_part(part_number, token):
    ...
    headers = {
        ...
        'X-DIGIKEY-Client-Id': 'DbHySAsTtSBo7pSc4OGzBkcg0891XbvaXAgNWS5xWheNsR7r',
    ...
    payload = {
        ...
        "SearchOptions": "ManufacturerProductNumber",
        "RecordCount": 10,
...
def get_footprint(parameters):
    ...
    for param in parameters:
        print(param)
        if param['ParameterId'] == 16:
            print(param['ValueText'])
...
def parse_exact_match(match):
    print("Found exact match!")    
    row = [
        match['ManufacturerProductNumber'],
        'Exact',
        '0',
        match['Manufacturer']['Name'],
        get_footprint(match.get('Parameters', [])),
        match['Description']['DetailedDescription']
    ]
    row[4] = string_cleanup(row[4])
    return(row)

def parse_close_matches(parts):
    
    close_matches = []
                    
    # Process each part in the response
    for part in parts:
        prod_var = part.get('ProductVariations', '')
        dk_part = prod_var[0]['DigiKeyProductNumber']
        manu_part = part['ManufacturerProductNumber']
        print(manu_part)
...
def process_csv(input_file, output_file):
    ...
    with open(input_file, 'r') as infile, open(output_file, 'w', newline='') as outfile:
        ...
        writer.writerow(['Original Part', 'Match Type', 'DigiKey Part', 
                       'Manufacturer', 'Footprint', 'Description'])
        ...
        for row_idx, row in enumerate(reader):
            ...
            try:
                ...
                if result.get('ExactMatches', []) is not None:
                    exact_match = result.get('ExactMatches', []) 
                    parsed = parse_exact_match(exact_match[0])
                    writer.writerow(parsed)
                elif result.get('Products', []) is not None:
                    parts = result.get('Products', [])                    
                    print(f"API returned {len(parts)} parts")
                    parsed = parse_close_matches(parts)
                    if writer.writerow(parsed)
